...
class SnakeGame:
    ...
    def draw_ui_layer(self, frame):
        ...
        # 绘制游戏边界
        xm, ym, xM, yM = self.play_area
        # outline颜色使用了和蛇身边框类似的浅蓝色，width=3表示线条宽度
        ...
        draw.text(((self.width - (tb_time[2] - tb_time[0])) // 2, bar_y + 25), time_str, font=self.font_md,
                  fill=(100, 100, 100))
        ...
        if self.mode == "single":
            ...
            draw.text((100, 55), current_level_text, font=self.font_md, fill=(100, 100, 100))

            # 左侧 P1
            draw_text_pill(draw, (20, 55), f"P1: {self.scores[0]}", self.font_md, text_color=(50, 200, 50))

            # 右侧 P2
            p2_text = f"P2: {self.scores[1]}"
            tb = draw.textbbox((0, 0), p2_text, font=self.font_md)
            p2_w = tb[2] - tb[0]
            draw_text_pill(draw, (self.width - p2_w - 40, 55), p2_text, self.font_md, text_color=(0, 200, 200))

            # 中间目标分数 (金色)
            target_str = f"GOAL: {DUAL_TARGET_LENGTH}"
            tb_tgt = draw.textbbox((0, 0), target_str, font=self.font_lg)
            t_w = tb_tgt[2] - tb_tgt[0]
            draw.text(((self.width - t_w) // 2, 60), target_str, font=self.font_lg, fill=(255, 165, 0))
        ...
        if any(self.game_overs) and self.result_text:
            ...
            draw.rounded_rectangle((cx - 200, cy - 100, cx + 200, cy + 100), radius=20, fill=(255, 255, 255, 230),
            ...
            cx, cy = self.width // 2, self.height // 2
            draw.text((cx - 100, cy), self.sub_text, font=self.font_lg, fill=(255, 215, 0), stroke_width=2,
                      stroke_fill="black")
    ...
    def select_mode(self) -> bool:
        ...
        while True:
            ...
            if menu_state == 'main':
                ...
                draw.rounded_rectangle((50, 100, 590, 180), radius=15, fill=(255, 240, 245), outline=(255, 182, 193),
                                       width=3)
                ...
                draw.rounded_rectangle((50, 200, 590, 280), radius=15, fill=(240, 248, 255), outline=(135, 206, 250),
                                       width=3)
                ...
                draw.rounded_rectangle((100, 90, 540, 160), radius=15, fill=(224, 255, 255), outline=(0, 200, 200),
                                       width=3)
                ...
                draw.rounded_rectangle((100, 180, 540, 250), radius=15, fill=(255, 228, 225), outline=(255, 100, 100),
                                       width=3)
